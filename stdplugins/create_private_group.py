    ...
    if type_of_group == "b":
        try:
            ...
            await borg(functions.messages.DeleteChatUserRequest(
                ...
                user_id="@ViperAdnanbot"
            ))
            result = await borg(functions.messages.ExportChatInviteRequest(
                peer=created_chat_id,
            ))
            await event.edit("**[Group]({}) `{}` created successfully.**".format(result.link, group_name))
        ...
        try:
            r = await borg(functions.channels.CreateChannelRequest(  # pylint:disable=E0602
                title=group_name,
                about="This chat is under maintenance.",
            ...
            await event.edit("**[Channel]({}) `{}` created successfully.**".format(result.link, group_name))
        except Exception as e:  # pylint:disable=C0103,W0703
            await event.edit(str(e))
    else:
